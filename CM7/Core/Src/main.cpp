...
HSEM notification */
	...
	int main_epilogue();
	return main_epilogue();
}

extern volatile bool qp_running;
...
int main_epilogue()
{
	static_cast<COMMON::GUI_Mgr *>(COMMON::AO_GUI_Mgr)->SetDsiHandle(&hdsi);

	static QSubscrList subscrSto[COMMON::MAX_PUB_SIG];
	static QF_MPOOL_EL(COMMON::LargestEvent) smlPoolSto[50];
	...
	static AOStart<10> startHighPriorityAos[] {
			AO_Blinky,
			COMMON::AO_GUI_Mgr
	};
